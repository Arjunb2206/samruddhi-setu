...
(function (global) {
  ...
  function suppliedImage(product) {
    ...
    const exact = Object.keys(SUPPLIED_BY_TITLE).find(k => title === k || title.includes(k) || k.includes(title));
    if (exact) return SUPPLIED_BY_TITLE[exact];
    ...
    const alias = aliases.find(([a,b]) => title.includes(a) && title.includes(b));
    return alias ? SUPPLIED_BY_TITLE[alias[2]] : "";
  }
  const IMAGE_BY_KEY = {
